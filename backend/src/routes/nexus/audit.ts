...
import {
  getPlatformAuditForActor,
  listPlatformAuditForActor,
} from "../../domains/audit/service.js";

/** Platform audit is read-only over HTTP; append is server-internal only. */
...
audit.get("/", async (c) => {
  ...
  const data = await listPlatformAuditForActor(admin, actor, {
...
audit.get("/:id", async (c) => {
  ...
  const data = await getPlatformAuditForActor(admin, actor, id);
  return c.json({ data });
});
