...
import {
  getLearnerTimetableForActor,
  getTeacherTimetableForActor,
} from "../../domains/timetable/portal.js";
...
/**
 * POST /api/v1/timetable/assignments
 * Staff create — canonical teacher × subject × section placement.
 */
timetable.post("/assignments", async (c) => {
  const actor = assertAuthenticated(c);
  const admin = requireAdmin(c);
  ...
  const data = await createAssignmentForActor(admin, actor, {
...
/**
 * GET /api/v1/timetable/portal/students/:studentId
 * Learner weekly schedule with subject/teacher labels.
 */
timetable.get("/portal/students/:studentId", async (c) => {
  const actor = assertAuthenticated(c);
  const admin = requireAdmin(c);
  ...
  const data = await getLearnerTimetableForActor(admin, actor, {
    instituteId: query.institute_id,
    studentId,
  });
  return c.json({ data });
...
/**
 * GET /api/v1/timetable/portal/teacher
 * Teacher weekly schedule with subject labels.
 */
timetable.get("/portal/teacher", async (c) => {
  const actor = assertAuthenticated(c);
  const admin = requireAdmin(c);
  const query = validateQuery(portalTeacherQuerySchema, c.req.query());
  const data = await getTeacherTimetableForActor(admin, actor, {
    instituteId: query.institute_id,
    teacherId: query.teacher_id,
    sectionId: query.section_id,
  });
