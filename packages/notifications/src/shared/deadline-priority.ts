/**
 * Centralized deadline → notification priority.
 *
 * Thresholds (no prior LumenX business constants existed for notification
 * presentation; documented here so Admin/Connect/backend stay aligned):
 * - more than 3 days remaining → normal (info / blue)
 * - within 3 days → important (warning / amber)
 * - within 24 hours, or past due → critical (urgent / red)
 *
 * Does not change business due dates — presentation only.
 */
...
/** Escalate stored priority from due_at. Success/critical are left unchanged. */
