...
import { resolveCalendarMonthBounds } from "./month-calendar";
...
function Calendar({
  ...
  const { startMonth: defaultStart, endMonth: defaultEnd } = resolveCalendarMonthBounds();
  const startMonth = startMonthProp ?? defaultStart;
  const endMonth = endMonthProp ?? defaultEnd;

  return (
    <DayPicker
      showOutsideDays={showOutsideDays}
      className={cn(
        "bg-background group/calendar p-3 [--cell-size:2.25rem] sm:[--cell-size:2rem] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent",
      ...
      classNames={{
        ...
        dropdowns: cn(
          "flex h-(--cell-size) w-full items-center justify-center gap-1.5 px-1 text-sm font-medium",
          defaultClassNames.dropdowns,
          classNames?.dropdowns,
        ),
        dropdown_root: cn(
          "has-focus:border-ring border-input shadow-xs has-focus:ring-ring/50 has-focus:ring-[3px] relative min-h-9 min-w-[4.5rem] rounded-md border touch-manipulation",
        ...
        range_start: cn("bg-primary/15 rounded-l-md", defaultClassNames.range_start, classNames?.range_start),
        range_middle: cn("bg-primary/10 rounded-none", defaultClassNames.range_middle, classNames?.range_middle),
        range_end: cn("bg-primary/15 rounded-r-md", defaultClassNames.range_end, classNames?.range_end),
