...
export function AppShell({
  ...
  return (
    <div className="max-w-[1600px] mx-auto w-full">
      <div key={pageKey} className="lx-page-header lx-page-header--enter mb-6 lg:mb-8">
        <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-end sm:justify-between gap-4">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl lg:text-[1.75rem] font-semibold tracking-tight leading-tight">
              {title}
            </h1>
            {subtitle && (
          ...
          {actions && <div className="lx-actions-bar hidden sm:flex shrink-0">{actions}</div>}
        </div>
      </div>
      ...
      {actions && (
        <>
          ...
          <div className="lx-mobile-actions-bar sm:hidden">{actions}</div>
        </>
      )}
    </div>
  );
