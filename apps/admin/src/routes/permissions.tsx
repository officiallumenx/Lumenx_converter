...
import { Plus, Shield, Edit3, Trash2, Copy, Save, Users as UsersIcon, Check } from "lucide-react";
...
const mkPerms = (
  preset: Partial<Record<Module, Perm>> = {},
  fallback: Perm = "none",
): Record<Module, Perm> =>
...
function PermissionsPage() {
  ...
  return (
    <AppShell
      ...
      subtitle={`IAM roles · ${PLAN_DETAILS[plan].customRoleCap === Infinity ? "Unlimited" : roleCap} custom roles on ${plan} plan`}
      actions={
        <Button
          variant="primary"
          onClick={() => setOpenCreate(true)}
          disabled={roles.filter((r) => !r.system).length >= roleCap}
        >
    ...
    >
      ...
      <Card>
        <CardHeader
          title="Role Matrix"
          hint="Green = full · Amber = read-only · Gray = no access"
          action={
            <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
              <Shield className="size-3" /> {roles.reduce((s, r) => s + r.users, 0)} admins
            </div>
          }
        />
                  ...
                  <td className="px-3 py-3">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        title="Duplicate"
                        className="size-7 rounded-md hover:bg-surface-hover flex items-center justify-center text-muted-foreground"
                      >
                      ...
                      {!r.system && (
                        <button
                          ...
                          title="Delete"
                          className="size-7 rounded-md hover:bg-surface-hover flex items-center justify-center text-destructive"
                        >
      ...
      <Card className="mt-6">
        <CardHeader
          title="Enterprise role capabilities"
          hint="Reference matrix · View · Create · Edit · Approve · Publish"
        />
        <div className="px-5 pb-5 overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-xs border-collapse">
            <thead>
              <tr className="text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">
                <th className="py-3 pr-4 font-semibold text-left">Role</th>
                {(Object.keys(CAPABILITY_LABELS) as Capability[]).map((c) => (
                  <th key={c} className="py-3 px-2 font-semibold text-center">
                    {CAPABILITY_LABELS[c]}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border">
              {ENTERPRISE_ROLES.map((role) => (
                <tr key={role.id} className="hover:bg-surface-hover">
                  <td className="py-3 pr-4 font-medium whitespace-nowrap">{role.name}</td>
...
function RoleEditor({
  ...
  const setPerm = (m: Module, p: Perm) =>
    setDraft((d) => ({ ...d, perms: { ...d.perms, [m]: p } }));
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Scope" hint="Branch, grade range, or department">
          <TextInput
            ...
            placeholder="Branch Alpha · Grades 9–12"
          />
        </Field>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              Set access per module. You can quick-fill with the presets below.
            </div>
          </div>
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {MODULES.map((m) => (
              <div
                key={m}
                className="grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_auto_auto_auto] items-center px-4 py-2.5 hover:bg-surface-hover"
              >
                <div className="text-xs font-medium">{m}</div>
                {(["none", "read", "full"] as Perm[]).map((p) => (
                  <label
                    key={p}
                    className={`flex items-center justify-center px-3 cursor-pointer ${p === "none" ? "hidden sm:flex" : ""} ${p === "read" ? "hidden sm:flex" : ""}`}
                  >
                    <input
                      type="radio"
                      name={`m-${m}`}
                      checked={draft.perms[m] === p}
                      onChange={() => setPerm(m, p)}
                    ...
                    >
                      {draft.perms[m] === p && (
                        <span
                          className={`size-2 rounded-full ${p === "full" ? "bg-success" : p === "read" ? "bg-warning" : "bg-muted-foreground"}`}
                        />
                      )}
                    </span>
                  </label>
                ...
                <select
                  value={draft.perms[m]}
                  onChange={(e) => setPerm(m, e.target.value as Perm)}
        ...
        <div className="flex items-center gap-3 mt-3 text-[10px] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="size-2 rounded-full bg-success" /> Full · create / edit / delete
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-2 rounded-full bg-warning" /> Read · view only
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-2 rounded-full bg-muted-foreground" /> None · hidden in sidebar
          </span>
        </div>
      </div>
      ...
      <div className="mt-5 p-3.5 rounded-lg border border-border bg-background/40 flex items-center gap-3">
        ...
        <div className="text-[11px] text-muted-foreground flex-1">
          Assign users to this role from the{" "}
          <span className="text-foreground font-medium">Accounts & Access</span> page after saving.
        </div>
      </div>
    </Modal>
  );
