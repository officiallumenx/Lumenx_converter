...
function TransportPage() {
  ...
  const openSos = TRANSPORT_SOS.filter(
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return TRANSPORT_VEHICLES.filter(
      (v) =>
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return TRANSPORT_ASSIGNMENTS.filter(
      (a) =>
        a.name.toLowerCase().includes(q.toLowerCase()) ||
        a.route.toLowerCase().includes(q.toLowerCase()),
    );
  }, [q]);
  ...
  return (
    ...
    >
      ...
      <Card className="mt-6">
        ...
        {tab === "routes" && (
          <>
            ...
            <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
              <span>
                Showing 1–{filteredRoutes.length} of {filteredRoutes.length}
              </span>
        ...
        {tab === "drivers" && (
          <>
                ...
                <tbody className="divide-y divide-border">
                  {filteredDrivers.map((d) => (
                    <tr key={d.id} className="hover:bg-surface-hover">
                      ...
                      <td className="px-5 py-3">
                        <Pill tone={d.compliance === "valid" ? "success" : "warning"}>
                          {d.compliance}
                        </Pill>
                      </td>
                    </tr>
            ...
            <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
              <span>
                Showing 1–{filteredDrivers.length} of {filteredDrivers.length}
              </span>
              <div className="flex gap-1">
                <Button size="sm" disabled>
                  Previous
        ...
        {tab === "assignments" && (
          <>
            ...
            <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
              <span>
                Showing 1–{filteredAssignments.length} of {filteredAssignments.length}
              </span>
              <div className="flex gap-1">
                <Button size="sm" disabled>
                  Previous
        ...
        {tab === "timings" && (
          <div className="px-5 pb-5 space-y-3">
            {routes.map((r) => (
              <div
                key={r.id}
                className="flex flex-wrap items-center justify-between p-4 rounded-lg border border-border bg-background/40"
              >
                <div>
        ...
        {tab === "sos" && (
          <div className="px-5 pb-5 space-y-3">
            {TRANSPORT_SOS.map((s) => (
              <div
                key={s.id}
                className="flex items-center gap-4 p-4 rounded-lg border border-destructive/20 bg-destructive/5"
              >
                <AlertTriangle className="size-4 text-destructive shrink-0" />
                <div className="flex-1">
                  <div className="text-xs font-medium">
                    {s.type} · Route {s.route}
                  </div>
                ...
                <Pill
                  tone={s.status === "resolved" || s.status === "closed" ? "success" : "danger"}
                >
                  {s.status}
                </Pill>
              </div>
        ...
        {tab === "compliance" && (
          <div className="px-5 pb-5">
            <CardHeader
              title="Driver compliance reports"
              hint="License, fitness, police verification"
            />
            <div className="space-y-2">
              {TRANSPORT_DRIVERS.map((d) => (
                <div
                  key={d.id}
                  className="flex items-center justify-between py-3 border-b border-border last:border-0 text-xs"
                >
                  <span className="font-medium">{d.name}</span>
                  <Pill tone={d.compliance === "valid" ? "success" : "warning"}>
                    {d.compliance === "valid" ? "All valid" : "Renew in 14d"}
                  </Pill>
          ...
          <div className="px-5 pb-5 space-y-3">
            {routes
              ...
              .map((r) => (
                <div
                  key={r.id}
                  className="flex flex-wrap items-center justify-between p-4 rounded-lg border border-border bg-background/40 text-xs"
                >
                  <div className="font-medium">Route {r.code} · Today</div>
                  <div className="text-muted-foreground">
                    On-time 94% · {r.students} students · 0 delays
                  </div>
      ...
      <Modal
        ...
        title="Create route"
        subtitle="Assign driver and vehicle after creation"
        size="lg"
        footer={
          <>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Morning pickup">
            <TextInput value={newMorning} onChange={(e) => setNewMorning(e.target.value)} />
          </Field>
          <Field label="Afternoon drop">
            <TextInput value={newAfternoon} onChange={(e) => setNewAfternoon(e.target.value)} />
          </Field>
        </div>
      </Modal>
    </AppShell>
