...
import { Card, CardHeader, Button, Pill, Kpi, Modal, Field, TextInput, SearchInput, SegmentedControl, PageToolbar, ToolbarSpacer, ToolbarMeta, DataTable, Th } from "@lumenx/ui-admin";
...
function FeesPage() {
  ...
  const totalPending = useMemo(() => students.reduce((a, s) => a + (s.total - s.paid), 0), [students]);
  const defaulters = useMemo(() => students.filter((s) => s.status === "overdue").length, [students]);
  ...
  const filteredStudents = useMemo(() => {
    let list = students;
    if (tab === "pending") list = list.filter((s) => s.status === "partial" || s.status === "overdue");
    else if (tab === "collected") list = list.filter((s) => s.status === "paid");
    if (q) list = list.filter((s) => s.name.toLowerCase().includes(q.toLowerCase()) || s.class.toLowerCase().includes(q.toLowerCase()));
    return list;
  }, [students, tab, q]);

  const addCategory = () => {
    if (!newName.trim()) return;
    setCats((p) => [...p, { id: `FC-${Date.now()}`, name: newName.trim(), students: 0, collected: "₹0", pending: "₹0" }]);
    setNewName("");
    setNewDesc("");
    setAddOpen(false);
  };
    ...
    <AppShell
      ...
      actions={
        <>
          ...
          <Button variant="primary" onClick={() => setAddOpen(true)}><Plus className="size-3.5" /> Add category</Button>
        </>
      }
    >
      ...
      <Card className="mt-6">
        <PageToolbar>
          ...
          {tab !== "categories" && (
            <>
              <SearchInput value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name or class…" className="flex-1 min-w-[200px]" />
              <ToolbarSpacer />
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add fee category"
        footer={<><Button onClick={() => setAddOpen(false)}>Cancel</Button><Button variant="primary" onClick={addCategory}>Add</Button></>}
      >
        <div className="space-y-4">
          <Field label="Category name" required><TextInput placeholder="Lab Fee" value={newName} onChange={(e) => setNewName(e.target.value)} /></Field>
          <Field label="Description"><TextInput placeholder="Annual lab usage charge" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} /></Field>
        </div>
      </Modal>
    </AppShell>
  );
