...
import { Archive, Trash2, Minimize2 } from "lucide-react";
...
function StoragePage() {
  ...
  const archive = (module: string) => {
    setRows((p) => p.map((r) => r.module === module && !r.protected ? { ...r, sizeGb: Math.round(r.sizeGb * 0.6), pct: Math.round(r.pct * 0.6) } : r));
  };
  ...
  const compress = (module: string) => {
    setRows((p) => p.map((r) => r.module === module ? { ...r, sizeGb: Math.round(r.sizeGb * 0.85) } : r));
  };

  const fmt = (gb: number) => gb >= 1024 ? `${(gb / 1024).toFixed(1)} TB` : `${gb} GB`;

  return (
    <AppShell title="Cloud Storage" subtitle={`Plan quota: ${fmt(quotaGb)} · manage by module`}>
      <div className="lx-kpi-grid">
        <Kpi label="Used" value={fmt(usedGb)} delta={`of ${fmt(quotaGb)}`} />
        <Kpi label="Available" value={fmt(availableGb)} delta={`${Math.round((availableGb / quotaGb) * 100)}% free`} tone="up" />
        ...
        <Card className="col-span-12 lg:col-span-7">
          ...
          <div className="px-5 pb-5">
            <div className="flex h-3 rounded-full overflow-hidden bg-muted">
              {rows.map((b) => <div key={b.module} className={b.color} style={{ width: `${b.pct}%` }} />)}
            </div>
            <div className="mt-5 space-y-2">
              {rows.map((b) => (
                <div key={b.module} className="flex flex-wrap items-center justify-between gap-2 py-2 border-b border-border last:border-0 text-xs">
                  <div className="flex items-center gap-2.5">
                    <span className={`size-2.5 rounded-sm ${b.color}`} />
                    <span className="font-medium">{b.module}</span>
                    {b.protected && <span className="text-[10px] text-muted-foreground">Protected</span>}
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-muted-foreground">{b.sizeGb} GB · {b.pct}%</span>
                    {!b.protected && (
                      <>
                        <Button size="sm" onClick={() => archive(b.module)}><Archive className="size-3" /> Archive</Button>
                        <Button size="sm" onClick={() => compress(b.module)}><Minimize2 className="size-3" /> Compress</Button>
                        {b.module === "Temp" && <Button size="sm" variant="danger" onClick={() => remove(b.module)}><Trash2 className="size-3" /> Delete</Button>}
                      </>
                    )}
                  </div>
                </div>
        ...
        <Card className="col-span-12 lg:col-span-5">
          ...
          <div className="px-5 pb-5 space-y-3 text-xs">
            <div className="p-4 rounded-lg border border-border bg-background/40">
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">Reclaim {tempGb} GB from uploads cache</div>
              <Button className="mt-2" size="sm" variant="danger" onClick={() => remove("Temp")}><Trash2 className="size-3" /> Clear temp</Button>
            </div>
            <div className="p-4 rounded-lg border border-warning/20 bg-warning/5">
              <div className="font-medium text-warning">At {Math.round((usedGb / quotaGb) * 100)}% capacity</div>
              <div className="text-[10px] text-muted-foreground mt-1">Archive Assignments or Media to stay within quota.</div>
            </div>
          </div>
        </Card>
      </div>
