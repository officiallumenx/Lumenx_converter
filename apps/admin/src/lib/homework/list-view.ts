...
import type { HomeworkListStatus } from "./load";

export type HomeworkInstituteGateStatus =
  | "demo"
  | "loading"
  | "ready"
  | "needs_selection"
...
export type ResolveHomeworkListViewInput = {
  apiMode: boolean;
  instituteStatus: HomeworkInstituteGateStatus;
  activeInstituteId: string | null;
  resolvedForInstituteId: string | null;
  storedItems: HomeworkListItem[];
  storedStatus: HomeworkListStatus;
...
export function resolveHomeworkListView(
  input: ResolveHomeworkListViewInput,
): HomeworkListView {
  if (!input.apiMode) {
    return {
      status: "demo",
...
export function shouldCommitHomeworkLoad(opts: {
  cancelled: boolean;
