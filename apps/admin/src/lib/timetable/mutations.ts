...
import type { TeacherAssignmentDto, TimetableSlotDto, TimetableSlotStatus } from "./types";
import { publishSectionTimetable as publishSectionTimetableApi } from "./api";
...
export async function publishSectionTimetable(
  ...
  assertApiMode();
  if (!isInstituteUuid(input.instituteId) || !isInstituteUuid(input.sectionId)) {
    throw new Error("institute_id and section_id must be valid UUIDs");
  }
  return publishSectionTimetableApi(input, client);
}
...
export async function createTeacherAssignment(
  ...
  if (!isInstituteUuid(input.teacherId)) {
    throw new Error("teacher_id must be a valid UUID");
  }
  return client.post<TeacherAssignmentDto>(
    "/api/v1/timetable/assignments",
    {
      institute_id: input.instituteId.trim(),
      academic_year_id: input.academicYearId.trim(),
      class_id: input.classId.trim(),
