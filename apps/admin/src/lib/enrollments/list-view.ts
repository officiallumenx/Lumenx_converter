...
import type { EnrollmentListStatus } from "./load";

export type EnrollmentInstituteGateStatus =
  | "demo"
  | "loading"
  | "ready"
  | "needs_selection"
...
type ResolveEnrollmentsListViewInput = {
  apiMode: boolean;
  instituteStatus: EnrollmentInstituteGateStatus;
  activeInstituteId: string | null;
  resolvedForInstituteId: string | null;
  storedItems: EnrollmentListItem[];
  storedStatus: EnrollmentListStatus;
  storedErrorMessage: string | null;
  instituteErrorMessage: string | null;
};

export type EnrollmentsListView = {
...
export function resolveEnrollmentsListView(
  input: ResolveEnrollmentsListViewInput,
): EnrollmentsListView {
  if (!input.apiMode) {
    return {
      status: "demo",
...
export function shouldCommitEnrollmentsLoad(opts: {
  cancelled: boolean;
