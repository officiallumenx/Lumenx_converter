/**
 * Pure helpers for API-mode audit list validity.
...
import type { AuditListStatus } from "./load";

export type AuditInstituteGateStatus =
  | "demo"
  | "loading"
  | "ready"
  | "needs_selection"
...
export type ResolveAuditListViewInput = {
  apiMode: boolean;
  instituteStatus: AuditInstituteGateStatus;
  activeInstituteId: string | null;
  resolvedForInstituteId: string | null;
  storedItems: AuditEntry[];
  storedStatus: AuditListStatus;
...
export function resolveAuditListView(
  input: ResolveAuditListViewInput,
): AuditListView {
  if (!input.apiMode) {
    return {
      status: "demo",
...
export function shouldCommitAuditLoad(opts: {
  cancelled: boolean;
