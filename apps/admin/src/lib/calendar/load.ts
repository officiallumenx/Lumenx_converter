/**
 * Dual-mode calendar list loader.
...
import { eventDtosToCalendarListItems } from "./map";
...
export async function loadCalendarList(
  activeInstituteId: string | null,
): Promise<CalendarListState> {
  if (!isApiAuthMode()) {
  ...
  try {
    const dtos = await listCalendarEvents({ instituteId: activeInstituteId });
    const items = eventDtosToCalendarListItems(dtos);
    return {
