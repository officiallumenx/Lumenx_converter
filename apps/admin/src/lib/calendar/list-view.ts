/**
 * Pure helpers for API-mode calendar list validity.
 * Prevents painting institute A's rows under institute B before effects run.
 */
import type { CalendarListItem } from "./types";
import type { CalendarListStatus } from "./load";

export type CalendarInstituteGateStatus =
  | "demo"
  | "loading"
  | "ready"
  | "needs_selection"
...
export type ResolveCalendarListViewInput = {
  apiMode: boolean;
  instituteStatus: CalendarInstituteGateStatus;
  activeInstituteId: string | null;
  resolvedForInstituteId: string | null;
  storedItems: CalendarListItem[];
  storedStatus: CalendarListStatus;
  storedErrorMessage: string | null;
  instituteErrorMessage: string | null;
};

export type CalendarListView = {
...
export function resolveCalendarListView(
  input: ResolveCalendarListViewInput,
): CalendarListView {
  if (!input.apiMode) {
    return {
      status: "demo",
...
export function shouldCommitCalendarLoad(opts: {
  cancelled: boolean;
