...
const EVENTS_PARTICIPATION = [
  {
    event: "Annual Science Symposium",
    date: "2026-05-22",
    audience: "All grades",
    rsvp: 412,
    attended: 398,
...
function datasetToHtml(data: ReportDataset) {
  ...
  const bodyRows = data.rows
    .map(
      (row) => `<tr>${data.headers.map((h) => `<td>${String(row[h] ?? "")}</td>`).join("")}</tr>`,
    )
    .join("");
  ...
  switch (reportId) {
    ...
    case "leave": {
      ...
      return {
        ...
        institute,
        generatedAt,
        summary: `${students.length + teachers.length} requests (demo)`,
        headers: ["id", "kind", "name", "from", "to", "days", "status"],
        rows: [
          ...students.map((r) => ({
            id: r.id,
            kind: "student",
    ...
    case "documents": {
      ...
      return {
        ...
        institute,
        generatedAt,
        headers: ["id", "title", "ownerType", "ownerName", "category", "verification", "expiryDate"],
        rows: docs.map((d) => ({
          id: d.id,
          title: d.title,
    ...
    case "audit": {
      ...
      return {
        ...
        institute,
        generatedAt,
        headers: ["id", "user", "role", "action", "target", "module", "status", "at"],
        rows: log.map((e) => ({
          id: e.id,
          user: e.user,
          role: e.role,
...
export function pushRecentExport(entry: Omit<RecentExport, "id" | "exportedAt">) {
  ...
  const prev = loadRecentExports().filter(
    (e) => !(e.reportId === item.reportId && e.format === item.format),
  );
