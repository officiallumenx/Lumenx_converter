...
import type { AttendanceConfigLoadStatus } from "./config-load";

export function resolveAttendanceConfigView(input: {
  apiMode: boolean;
  instituteStatus:
    | "demo"
    | "loading"
  ...
  storedItems: AttendanceConfigDto[];
  storedStatus: AttendanceConfigLoadStatus;
...
export function shouldCommitAttendanceConfigLoad(opts: {
  cancelled: boolean;
