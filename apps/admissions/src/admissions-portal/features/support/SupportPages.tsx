...
export function DocumentCenterPage() {
  ...
  const { applications: apiApps } = useAdmissionsApplications();
  const [tick, setTick] = useState(0);
  const demoApps = user ? getApplicationsForUser(user.id).filter((a) => a.status !== "draft") : [];
  const apps = apiMode ? apiApps.filter((a) => a.status !== "draft") : demoApps;
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        {isInstituteAdmin && (
          <div className="mt-6 space-y-4 rounded-xl border border-border bg-muted/30 p-4">
            ...
            <Button asChild>
              <Link to="/institute">Open institute dashboard</Link>
            </Button>
          </div>
        )}
...
export function AdmissionsSettingsPage() {
  ...
  const sections: {
    ...
    },
    {
      title: "Help & support",
      items: [
        { label: "FAQs", to: "/faq", desc: "Common admission questions" },
        { label: "Help center", to: "/contact", desc: "Contact admissions support" },
      ],
    },
    {
      title: "Legal",
