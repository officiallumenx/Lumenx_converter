...
export function ApplyWizardPage({
  ...
  const submitAllChildren = async () => {
    ...
    for (const child of children) {
      ...
      try {
        if (apiMode) {
          const openings = await listAdmissionOpenings({ instituteId: resolvedInstituteId });
          const openingId = resolveOpeningIdForProgram(openings, child.programId!);
          if (!openingId) {
            toast.error("No admission opening is available for the selected program.");
            return;
          }
          const created = await createAdmissionApplication({
            ...
            payload: {
              ...
              documents: Object.entries(child.documents ?? {}).map(([type, value]) => ({
                id: `doc-${type}`,
                type,
      ...
      <div className="mx-auto max-w-md py-12 text-center animate-in fade-in">
        ...
        <div className="mt-8 flex flex-col gap-2">
          ...
          <Button variant="outline" asChild>
            <Link to="/apply">Apply for another child</Link>
          </Button>
        </div>
      </div>
    );
