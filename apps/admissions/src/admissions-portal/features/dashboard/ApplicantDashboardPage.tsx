...
export function ApplicantDashboardPage() {
  ...
  const { applications: apps } = useAdmissionsApplications();
  if (!user) return null;
    ...
    <div className="animate-in fade-in duration-300 space-y-8">
      ...
      <SectionCard title="Quick actions">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Apply now", to: "/apply", icon: FilePlus },
            { label: "Upload documents", to: "/documents", icon: Upload },
            { label: "Track status", to: "/applications", icon: FolderOpen },
            { label: "Contact admissions", to: "/inquiries", icon: MessageSquare },
          ].map((a) => (
      ...
      <div className="rounded-2xl bg-primary/10 border border-primary/20 p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button size="sm" variant="outline" asChild>
          <Link to="/inquiries">Open inquiry center</Link>
        </Button>
      </div>
    </div>
  );
