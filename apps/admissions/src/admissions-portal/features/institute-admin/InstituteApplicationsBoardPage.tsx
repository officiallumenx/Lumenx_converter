...
import { transitionAdmissionApplication } from "@/lib/admissions/api";
import type { AdmissionApplicationStatus } from "@/lib/admissions/api/types";
import {
  adminStageToStatus,
...
export function InstituteApplicationsBoardPage() {
  ...
  const { openings: apiOpenings, reload: reloadOpenings } = useAdmissionsOpenings({
    instituteId,
  });
  ...
  const apps = useMemo(() => {
    void tick;
    if (apiMode) return apiApps;
    return getApplicationsForInstitute(instituteId, getAllApplications());
  }, [apiApps, apiMode, instituteId, tick]);

  const openings = useMemo(() => {
    void tick;
    if (apiMode) return apiOpenings;
    return getOpeningsForInstitute(instituteId);
  }, [apiMode, apiOpenings, instituteId, tick]);
  ...
  const moveToStage = (id: string, stage: BoardStage, message: string) => {
    ...
    if (apiMode) {
      void transitionAdmissionApplication(id, { status: status as AdmissionApplicationStatus })
        .then(() => {
          reloadApps();
          reloadOpenings();
          setTick((t) => t + 1);
          ...
          if (stage === "approved") {
            toast.message("Next step", {
              description:
                "Open LumenX Admin → Admissions to convert this applicant into a student.",
            });
          }
        })
        .catch(() => {
      ...
      <section className="rounded-2xl border border-border bg-card p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link to="/institute/openings">Manage openings</Link>
          </Button>
        </div>
