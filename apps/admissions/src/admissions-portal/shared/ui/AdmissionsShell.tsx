...
const PRIMARY_NAV: NavItem[] = [
  ...
  { to: "/apply", label: "Apply", icon: FilePlus, auth: true, parentOnly: true },
];
...
const MORE_NAV: NavItem[] = [
  ...
  { to: "/documents", label: "Documents", icon: FileText, auth: true, parentOnly: true },
  {
    to: "/inquiries",
    label: "Inquiries",
    icon: MessageSquare,
...
const INSTITUTE_ADMIN_NAV: NavItem[] = [
  { to: "/institute", label: "Dashboard", icon: LayoutDashboard, exact: true },
  { to: "/institute/applications", label: "Applications", icon: FolderOpen },
  { to: "/institute/openings", label: "Openings", icon: DoorOpen },
  { to: "/institute/form", label: "Application form", icon: FormInput },
  { to: "/institute/profile", label: "Institute profile", icon: Building2 },
];
...
function isActive(pathname: string, to: string, exact?: boolean) {
  ...
  if (to === "/institutes") {
    return pathname === to || pathname.startsWith("/institutes/");
  }
  if (to === "/institute") {
    return pathname === to || pathname === `${to}/`;
  }
  if (to === "/institute/profile" || to === "/institute/settings") {
    return (
      pathname === "/institute/profile" || pathname === "/institute/settings"
    );
  }
  if (to === "/institute/openings") {
    return pathname === to || pathname.startsWith(`${to}/`);
  }
  if (to.startsWith("/institute/")) {
    return pathname === to || pathname.startsWith(`${to}/`);
  }
  return pathname === to || pathname.startsWith(`${to}/`);
}
          ...
          {user ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button variant="outline" className="w-full" asChild>
                <Link to="/login" search={{ type: "parent" }}>
                  Parent login
                </Link>
              </Button>
              <Button variant="ghost" className="w-full" asChild>
                <Link to="/login" search={{ type: "institute" }}>
                  Institute access
                </Link>
              </Button>
            </div>
