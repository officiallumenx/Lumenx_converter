...
/** Persist API-authenticated careers user (Supabase session is authoritative). */
export function persistApiCareersUser(user: CareersUser): void {
  setCurrentUser({
    ...normalizeCareersUser({
      ...user,
      authSource: "api",
      passwordHash: "",
