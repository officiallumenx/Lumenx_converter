...
import { Button, Badge, Input } from "@lumenx/ui";
...
export function InstituteProfilePage({ instituteId }: { instituteId: string }) {
  ...
  const [inst, setInst] = useState(() => getInstituteProfile(instituteId));
  const [followed, setFollowed] = useState(() =>
    user ? isInstituteFollowed(user.id, instituteId) : false,
  );
  const [saved, setSaved] = useState(() => (user ? isInstituteSaved(user.id, instituteId) : false));

  useEffect(() => {
    let cancelled = false;
    void getInstituteProfileWithApiFallback(instituteId).then((next) => {
      if (!cancelled && next) setInst(next);
    });
    return () => {
      cancelled = true;
    };
    ...
    return (
      <div className="text-center py-16">
        ...
        <Button className="mt-4" variant="outline" asChild>
          <Link to="/institutes">Back to institutes</Link>
        </Button>
      </div>
    );
  }
  ...
  const toggleFollow = () => {
    if (!user) {
      toast.message("Sign in to follow institutes");
      return;
    }
  ...
  const toggleSave = () => {
    if (!user) {
      toast.message("Sign in to save institutes");
      return;
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" size="sm" onClick={toggleFollow}>
              <Heart
                className={cn("size-4 mr-1", followed && "fill-destructive text-destructive")}
              />
              {followed ? "Following" : "Follow"}
            </Button>
            <Button variant="secondary" size="sm" onClick={toggleSave}>
              {saved ? "Saved" : "Save"}
            </Button>
            <Button size="sm" asChild>
              <Link to="/jobs" search={{ institute: instituteId }}>
                View jobs
              </Link>
            </Button>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-border p-5 space-y-3">
            ...
            <p className="text-sm text-muted-foreground italic leading-relaxed">
              &ldquo;{inst.principalMessage}&rdquo;
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {inst.gallery.map((g) => (
                <div
                  key={g.id}
                  className={cn(
                    "aspect-video rounded-xl bg-gradient-to-br flex items-end p-2",
                    g.gradient,
                  )}
                >
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-border p-4 space-y-3">
            <h3 className="font-semibold text-sm flex items-center gap-2">
              <Building2 className="size-4" /> Contact
            </h3>
      ...
      <section>
        ...
        {jobs.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No open roles right now. Follow to get alerts.
          </p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2">
            {jobs.map((job) => (
