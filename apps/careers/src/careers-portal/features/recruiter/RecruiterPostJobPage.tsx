...
export function RecruiterPostJobPage({ editJobId }: { editJobId?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    nav({ to: "/recruiter/jobs/$jobId/edit", params: { jobId: created.id } });
  };
        ...
        <div className="flex gap-3 pt-2">
          ...
          {isEdit && (
            <Button type="button" variant="outline" asChild>
              <Link to="/jobs/$jobId" params={{ jobId: editJobId! }}>
                Preview listing
              </Link>
            </Button>
          )}
          <Button type="button" variant="outline" asChild>
            <Link to="/recruiter/jobs">{isEdit ? "Back to my jobs" : "Cancel"}</Link>
          </Button>
        </div>
      </form>
    </div>
