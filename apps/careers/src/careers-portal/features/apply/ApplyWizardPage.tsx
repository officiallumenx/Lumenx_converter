    ...
    return (
      <div className="mx-auto max-w-lg text-center animate-in fade-in py-8">
        ...
        <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-center">
          <Button asChild>
            <Link to="/applications/$applicationId" params={{ applicationId: submittedId }}>
              Track application
            </Link>
          </Button>
  ...
  return (
    <div className="animate-in fade-in duration-300 max-w-2xl">
      <CareersPageHeader
        ...
        backTo={selectedJob ? `/jobs/${selectedJob.id}` : "/jobs"}
      />
      ...
      {selectedJob && missing.length === 0 && (
        <div className="mb-6 rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
          ...
          <Button variant="link" className="h-auto p-0 text-xs" asChild>
            <Link to="/profile">Update profile before applying</Link>
          </Button>
        </div>
      )}
