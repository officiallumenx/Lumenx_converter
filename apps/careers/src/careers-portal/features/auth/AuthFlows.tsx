...
export function SignInFlow({ redirect, job }: { redirect?: string; job?: string }) {
  ...
  const submit = () => {
    if (step === "identifier") {
      ...
      if (!r.success) return toast.error(isApiAuthMode() ? "Enter your email" : "Enter mobile or email");
      setStep("password");
      return;
    }
    setLoading(true);
      ...
      .then((loggedIn) => {
        if (loggedIn) {
          ...
          if (redirect) {
            nav({
              to: redirect as "/",
              search: redirect === "/apply" && job ? { job } : undefined,
            });
          } else {
  ...
  return (
    <div className="mx-auto max-w-md animate-in fade-in duration-300">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {isApiAuthMode()
          ? "Institute staff and applicants use the same LumenX account — sign in with your email."
