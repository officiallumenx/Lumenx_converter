...
export const Route = createFileRoute("/careers/setup-from-admin")({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setError("Could not finish setup. Try again from Admin, or sign in to Careers.");
        return;
      }
      ...
      if (dest === "applicants") {
        nav({ to: "/careers/recruiter/applicants", replace: true });
      } else if (dest === "jobs") {
        nav({ to: "/careers/recruiter/jobs", replace: true });
      } else {
        nav({ to: "/careers/recruiter", replace: true });
      }
    };
