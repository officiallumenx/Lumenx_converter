...
function schoolEventKindMeta(kind: string | null | undefined) {
    ...
    <div className="min-w-0 max-w-full">
      ...
      <div className="my-4 flex min-w-0 flex-wrap gap-2">
        {FILTERS.map((f) => (
          ...
          >
            {f === "all" ? "All" : schoolEventKindMeta(f).label}
          </button>
        ))}
      </div>
...
function CountdownBanner({ event }: { event: SchoolEvent }) {
  ...
  const meta = schoolEventKindMeta(event.kind);
...
function EventRow({ e }: { e: SchoolEvent }) {
  const meta = schoolEventKindMeta(e.kind);
  const Icon = meta.icon;
  return (
