...
export function InquiryCenterPage() {
  ...
  return (
    <div className="animate-in fade-in duration-300 space-y-8">
      <AdmissionsPageHeader title="Inquiry center" subtitle="Ask admission, program, fee, transport, or hostel questions" />
      ...
      <SectionCard title="New inquiry">
        <div className="space-y-4 max-w-xl">
          ...
          <div>
            ...
            <Textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Your question…" className="mt-1 min-h-[100px]" />
          </div>
      ...
      <SectionCard title="Inquiry history">
        {items.length === 0 ? (
          <EmptyState title="No inquiries yet" hint="Your questions and responses will appear here." />
        ) : (
            ...
            {items.map((inq) => (
              <div key={inq.id} className="rounded-xl border border-border p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Badge variant={inq.status === "answered" ? "default" : inq.status === "closed" ? "secondary" : "outline"}>
                    {inq.status}
                  </Badge>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">{inq.message}</p>
                {inq.responses.map((r) => (
                  <div key={r.id} className="mt-3 rounded-lg bg-primary/5 border border-primary/15 p-3 text-sm">
                    <p className="font-medium text-xs text-primary">{r.from}</p>
                    <p className="mt-1">{r.body}</p>
                    <p className="text-[10px] text-muted-foreground mt-1">{new Date(r.at).toLocaleString("en-IN")}</p>
                  </div>
                ))}
              </div>
            ))}
