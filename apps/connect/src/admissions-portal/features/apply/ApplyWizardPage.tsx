...
import {
  ...
  DocumentUploadCard,
} from "@/admissions-portal/shared/ui/AdmissionsShellWidgets";
    ...
    return (
      <div className="mx-auto max-w-md py-12 text-center animate-in fade-in">
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          Track status and upload documents anytime.
        </p>
      ...
      <div className="space-y-4 rounded-2xl border border-border bg-card p-4 sm:p-6">
          ...
          <>
            <Field label="Student name">
              <Input
                ...
                onChange={(e) => update({ student: { ...draft.student, name: e.target.value } })}
              />
            </Field>
            <Field label="Gender">
              <Select
                value={draft.student.gender ?? ""}
                onValueChange={(v) => update({ student: { ...draft.student, gender: v } })}
              >
                <SelectTrigger>
            ...
            <Field label="Date of birth">
              <Input
                ...
                onChange={(e) =>
                  update({ student: { ...draft.student, dateOfBirth: e.target.value } })
                }
              />
            </Field>
            <Field label="Nationality">
              <Input
                value={draft.student.nationality ?? "Indian"}
                onChange={(e) =>
                  update({ student: { ...draft.student, nationality: e.target.value } })
                }
              />
            </Field>
            <Field label="Blood group">
              <Select
                ...
                onValueChange={(v) => update({ student: { ...draft.student, bloodGroup: v } })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select" />
                </SelectTrigger>
          ...
          <>
            <Field label="Father's name">
              <Input
                ...
                onChange={(e) =>
                  update({ parent: { ...draft.parent, fatherName: e.target.value } })
                }
              />
            </Field>
            <Field label="Mother's name">
              <Input
                ...
                onChange={(e) =>
                  update({ parent: { ...draft.parent, motherName: e.target.value } })
                }
              />
            </Field>
            ...
            <Field label="Mobile">
              <Input
                ...
                onChange={(e) => update({ parent: { ...draft.parent, mobile: e.target.value } })}
              />
            </Field>
            <Field label="Email">
              <Input
                ...
                onChange={(e) => update({ parent: { ...draft.parent, email: e.target.value } })}
              />
            </Field>
            <Field label="Occupation">
              <Input
                ...
                onChange={(e) =>
                  update({ parent: { ...draft.parent, occupation: e.target.value } })
                }
              />
            </Field>
          </>
          ...
          <>
            <Field label="Address">
              <Input
                ...
                onChange={(e) => update({ address: { ...draft.address, address: e.target.value } })}
              />
            </Field>
            <Field label="City">
              <Input
            ...
            <Field label="Country">
              <Input
                ...
                onChange={(e) => update({ address: { ...draft.address, country: e.target.value } })}
              />
            </Field>
            <Field label="Postal code">
              <Input
                ...
                onChange={(e) =>
                  update({ address: { ...draft.address, postalCode: e.target.value } })
                }
              />
            </Field>
          </>
          ...
          <>
            <Field label="Current school">
              <Input
                ...
                onChange={(e) =>
                  update({ academic: { ...draft.academic, currentSchool: e.target.value } })
                }
              />
            </Field>
            <Field label="Current grade">
              <Input
                ...
                onChange={(e) =>
                  update({ academic: { ...draft.academic, currentGrade: e.target.value } })
                }
              />
            </Field>
            <Field label="Previous results">
              <Input
                value={draft.academic.previousResults ?? ""}
                onChange={(e) =>
                  update({ academic: { ...draft.academic, previousResults: e.target.value } })
                }
              />
            </Field>
            <Field label="Academic performance">
              <Input
                ...
                onChange={(e) =>
                  update({ academic: { ...draft.academic, performance: e.target.value } })
                }
              />
            </Field>
          </>
          ...
          <>
            <Field label="Program">
              <Select
                ...
                onValueChange={(v) => update({ programId: v, grade: "" })}
              >
                <SelectTrigger>
            ...
            <Field label="Academic year">
              <Input
                value={draft.academicYear ?? "2026–27"}
          ...
          <div className="space-y-3">
            {DOC_TYPES.map((d) => (
              <DocumentUploadCard
                ...
                onUpload={(f) =>
                  update({ documents: { ...draft.documents, [d.type]: { fileName: f.name } } })
                }
              />
            ))}
          </div>
        ...
        {step === 6 && (
          <div className="space-y-3 text-sm">
            ...
            <ReviewRow
              label="Program"
              value={`${selectedProgram?.name} · ${draft.grade}`}
              onEdit={() => setStep(4)}
            />
            <ReviewRow
              label="Parent mobile"
            ...
            <ReviewRow
              label="Documents"
              value={`${Object.keys(draft.documents).length} uploaded`}
              onEdit={() => setStep(5)}
            />
