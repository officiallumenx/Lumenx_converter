...
import { STUDENT_FAQS, STUDENT_HELP_TOPICS, SUPPORT_EMAIL } from "./support-content";

export function StudentFaqDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  return (
...
export function StudentHelpCenterDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85dvh] overflow-y-auto rounded-2xl sm:max-w-lg">
        ...
        <Accordion type="single" collapsible className="w-full">
          {STUDENT_HELP_TOPICS.map((topic, i) => (
...
export function StudentContactSupportDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const mailto = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent("LumenX Connect — Student Support")}`;
...
export function StudentFeedbackDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const submit = () => {
    ...
    toast.success("Thank you — your feedback was sent", {
      description: "Our team reviews student feedback regularly.",
    });
    setMessage("");
    setCategory("general");
    onOpenChange(false);
          ...
          <div>
            ...
            <Select value={category} onValueChange={setCategory}>
              ...
              <SelectContent position="popper" className="z-[100]">
                ...
                <SelectItem value="academics">Academics & marks</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
...
export function StudentReportIssueDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const submit = () => {
    ...
    window.location.href = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(`[Student Issue] ${subject.trim()}`)}&body=${body}`;
      ...
      <DialogContent className="rounded-2xl sm:max-w-md">
        ...
        <div className="space-y-3">
          <Input
            ...
            placeholder="Brief title (e.g. Marks not loading)"
