...
const CAT_LABEL = { academic: "Academic", sports: "Sports", program: "Program", holiday: "Holiday" };
...
export function TeacherEventsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    teacherRepository.getEvents().then((e) => { setEvents(e); setLoading(false); });
  }, [portal.isTeacher]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Events" subtitle="View school programs, sports, and academic events — managed by administration" />
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {filtered.map((ev) => (
            <article key={ev.id} className="rounded-2xl border bg-card p-4 shadow-soft transition-shadow hover:shadow-elevated sm:p-5">
              ...
              <div className="mt-4">
                <Button size="sm" variant="outline" className="rounded-lg" onClick={() => setSelected(ev)}>
                  View details
                </Button>
              </div>
            </article>
        ...
        <EmptyState icon={CalendarDays} title="No events" description="Events are published by the administration." />
      )}
          ...
          {selected && (
            <div className="space-y-4 text-sm">
              <div className="rounded-xl border bg-muted/20 p-4">
                ...
                <p className="mt-2 text-muted-foreground">{CAT_LABEL[selected.category]} · Organized by {selected.createdBy}</p>
              </div>
              <div className="rounded-xl border bg-muted/20 p-4">
                <h4 className="mb-2 font-semibold">Schedule & location</h4>
                <p className="flex items-center gap-2"><CalendarDays className="size-4 text-primary" /> {selected.date}</p>
                <p className="mt-1 flex items-center gap-2"><Clock className="size-4 text-primary" /> {selected.time}</p>
                <p className="mt-1 flex items-center gap-2"><MapPin className="size-4 text-primary" /> {selected.location}</p>
              </div>
            </div>
          )}
        </DialogContent>
