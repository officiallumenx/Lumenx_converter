...
export function StudentDetailPanel({
  ...
  return (
    <div className={cn("space-y-4", compact && "space-y-3")}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <DetailSection icon={Users} title="Parent & contact" compact={compact}>
          <dl className="space-y-2 text-sm">
            ...
            {detail.parentEmail ? <Row label="Email" value={detail.parentEmail} icon={Mail} /> : null}
        ...
        <DetailSection icon={ClipboardCheck} title="Attendance" compact={compact}>
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            ...
            <StatBox label="Absent" value={String(detail.attendanceSummary.daysAbsent)} tone="destructive" />
          </div>
          {detail.attendanceSummary.recentAbsences.length ? (
            <ul className="mt-2 space-y-1 text-sm">
              {detail.attendanceSummary.recentAbsences.map((a) => (
                <li key={a.date} className="flex justify-between rounded-lg bg-background/80 px-2.5 py-1.5">
                  <span>{a.date}</span>
        ...
        <DetailSection icon={TrendingUp} title="Academic performance" compact={compact}>
          <ul className="space-y-1.5">
            {detail.marks.map((m, i) => (
              <li key={i} className="flex items-center justify-between rounded-lg border border-border/80 bg-background/60 px-3 py-2 text-sm">
                <div>
                  <div className="font-medium">{m.exam}</div>
                  <div className="text-xs text-muted-foreground">{m.subject}</div>
                </div>
                <div className="text-right">
                  <div className="font-semibold tabular-nums">{m.total}% · {m.grade}</div>
          ...
          {detail.pendingWork.length ? (
            <ul className="space-y-1.5">
              {detail.pendingWork.map((w) => (
                <li key={w.id} className="flex items-start justify-between gap-2 rounded-lg border border-border/80 bg-background/60 px-3 py-2 text-sm">
                  <div className="min-w-0">
                    <div className="font-medium line-clamp-2">{w.title}</div>
                    <div className="text-xs capitalize text-muted-foreground">{w.type} · Due {w.dueLabel}</div>
                  </div>
                  <Badge
                    variant="outline"
                    className={cn(
      ...
      {(detail.achievements.length > 0 || detail.awards.length > 0 || detail.certificates.length > 0) && (
        <div className="grid gap-3 sm:grid-cols-2">
            ...
            <DetailSection icon={FileText} title="Certificates" compact={compact}>
              <ul className="space-y-1.5 text-sm">
                {detail.certificates.map((c, i) => (
                  <li key={i} className="flex justify-between rounded-lg bg-background/60 px-3 py-2">
                    <span>{c.title}</span>
...
function DetailSection({
  ...
  return (
    <section className={cn("rounded-xl border border-border/80 bg-card/80 p-3", !compact && "sm:p-4")}>
