...
export function TeacherAssignmentsPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([
      teacherRepository.getAssignments(classFilter === "all" ? undefined : classFilter),
      teacherRepository.getHomeworkClassSummaries(),
    ]).then(([a, s]) => {
      setAssignments(a);
      setClassSummaries(s);
      setLoading(false);
  ...
  const availableSections = useMemo(() => {
    const cls = portal.isTeacher ? portal.classes.filter((c) => classFilter === "all" || c.id === classFilter) : [];
    const secs = [...new Set(cls.map((c) => c.section))].sort();
    return secs;
  }, [portal, classFilter]);
  ...
  const displayAssignments = useMemo(() => {
    const list = assignments
      ...
      .filter((a) => {
        ...
        return cls?.section === sectionFilter;
      });
    if (sortBy === "dueDate") {
      return [...list].sort((a, b) => (a.dueDate < b.dueDate ? -1 : 1));
    }
    return [...list].sort((a, b) => a.title.localeCompare(b.title));
  }, [assignments, categoryType, statusFilter, sectionFilter, sortBy, portal]);
  ...
  return (
    <div className="min-w-0 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:gap-3">
        {[
          { label: "Active", value: String(assignments.length), icon: BookOpen },
          { label: "Drafts", value: String(pendingCount), icon: ClipboardList, tone: "warning" as const },
          { label: "Expired", value: String(expiredCount), icon: AlertTriangle, tone: "warning" as const },
          { label: "Avg submission %", value: `${avgSubmission}%`, icon: TrendingUp, tone: "primary" as const },
          {
            label: "Below 70%",
      ...
      <div className="flex gap-2">
        {(["assignment", "homework"] as const).map((t) => (
          <button key={t} type="button" onClick={() => setCategoryType(t)} className={cn("rounded-full px-4 py-2 text-sm font-medium capitalize", categoryType === t ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
      ...
      <Tabs defaultValue="assignments" className="w-full">
        ...
        <TabsContent value="assignments" className="mt-4 space-y-4">
          <div className="flex flex-wrap gap-2">
            <Select value={classFilter} onValueChange={(v) => { setClassFilter(v); setSectionFilter("all"); }}>
              <SelectTrigger className="h-10 w-[12rem] rounded-xl">
            ...
            {availableSections.length > 1 && (
              <Select value={sectionFilter} onValueChange={setSectionFilter}>
                <SelectTrigger className="h-10 w-[9rem] rounded-xl"><SelectValue placeholder="All sections" /></SelectTrigger>
                <SelectContent position="popper" className="z-[100]">
            ...
            <Select value={sortBy} onValueChange={(v) => setSortBy(v as "dueDate" | "title")}>
              <SelectTrigger className="h-10 w-[10rem] rounded-xl"><SelectValue /></SelectTrigger>
              <SelectContent position="popper" className="z-[100]">
          ...
          {loading ? (
            ...
            <AssignmentList assignments={displayAssignments} classes={portal.classes} onReload={load} />
          )}
        </TabsContent>
        ...
        <TabsContent value="submissions" className="mt-4">
          <SubmissionsTab assignments={assignments} loading={loading} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function AssignmentList({
  ...
  const publishFn = useCallback(async (id: string) => {
  ...
  if (!assignments.length) {
    return (
      <EmptyState icon={BookOpen} title="No items match" description="Try a different status filter or create a new assignment." />
    );
  }
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-2">
        {assignments.map((a) => {
          ...
          const isOverdue = !isExpired && dueDate && dueDate < now && a.publishStatus === "published";
          return (
          <article
            key={a.id}
            className={cn(
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className={cn("grid size-10 place-items-center rounded-xl bg-primary/10 text-primary", isExpired && "opacity-60")}><BookOpen className="size-5" /></div>
              <div className="flex flex-wrap gap-1">
                <Badge variant="outline" className="capitalize text-[10px]">{a.type}</Badge>
                <PublishStatusBadge status={a.publishStatus} />
                {isOverdue && <Badge className="border-0 bg-warning/15 text-warning-foreground text-[10px]">Overdue</Badge>}
              </div>
            </div>
            <h3 className="mt-3 font-semibold leading-snug line-clamp-2">{a.title}</h3>
            <p className="mt-1 text-xs text-muted-foreground">{a.subject} · Class {a.classLabel}</p>
            <div className={cn("mt-3 flex items-center gap-1.5 text-xs", isOverdue ? "text-warning-foreground font-medium" : "text-muted-foreground")}>
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={cn("font-semibold tabular-nums", a.submissionRate >= 80 ? "text-success" : a.submissionRate >= 60 ? "text-warning-foreground" : "text-destructive")}>{a.submissionRate}%</span>
              </div>
              <Progress value={a.submissionRate} className="h-2" />
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              <Button size="sm" variant="outline" className="rounded-lg" onClick={() => setView(a)}>View</Button>
              {!isExpired && (
                <>
                  <Button size="sm" variant="outline" className="rounded-lg" onClick={() => setEdit(a)}>Edit</Button>
                  {a.publishStatus === "draft" && (
                    <Button size="sm" className="rounded-lg" disabled={actionPending} onClick={() => publish(a.id)}>
                      {publishing ? "Publishing…" : "Publish"}
                    </Button>
                  )}
                </>
              )}
              <Button size="sm" variant="outline" className="rounded-lg text-destructive" disabled={actionPending} onClick={() => deleteAssignment(a.id)}>
        ...
        <DialogContent className="max-h-[90dvh] overflow-y-auto">
          ...
          {view && (
            <div className="space-y-3 text-sm">
              <p><span className="text-muted-foreground">Status:</span> <PublishStatusBadge status={view.publishStatus} /> · {view.status}</p>
              <p><span className="text-muted-foreground">Class:</span> {view.classLabel} · {view.subject}</p>
              <p><span className="text-muted-foreground">Due:</span> {view.due}</p>
              <div><span className="text-muted-foreground">Instructions:</span><p className="mt-1 whitespace-pre-wrap">{view.instructions || view.description}</p></div>
            </div>
          )}
        </DialogContent>
      </Dialog>

      {edit && <EditAssignmentDialog assignment={edit} classes={classes} onClose={() => setEdit(null)} onSaved={() => { setEdit(null); onReload(); }} />}
    </>
  );
}
...
function EditAssignmentDialog({
  ...
  const saveFn = useCallback(async () => {
    await teacherRepository.updateAssignment(assignment.id, { title, description, instructions: description, due: dueDate, dueDate });
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {summaries.map((s) => (
          ...
          >
            ...
            <div className="mt-2 font-display text-2xl font-semibold tabular-nums text-primary">
              {s.avgSubmissionPct}%
            </div>
            <div className="mt-1 text-[11px] text-muted-foreground">
              On-time {s.avgOnTimePct}% · {s.studentsBelow70} below 70%
            </div>
          </button>
        ))}
      </div>
            ...
            {rowsLoading ? (
              ...
              rows.map((r) => (
                <TableRow key={r.studentId}>
                  ...
                  <TableCell className="text-right tabular-nums text-warning-foreground">{r.late}</TableCell>
                  <TableCell className="text-right tabular-nums text-destructive">{r.missing}</TableCell>
...
function SubmissionsTab({
  ...
  useEffect(() => {
    if (assignments.length && !assignmentId) setAssignmentId(assignments[0].id);
  }, [assignments, assignmentId]);

  useEffect(() => {
    if (!assignmentId) return;
    setSubsLoading(true);
    teacherRepository.getAssignmentSubmissions(assignmentId).then((s) => {
      setSubmissions(s);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-soft sm:p-5">
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Filter by assignment, student, and submission timing (early, on time, late, missing).
      </p>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
        <Field label="Assignment">
          <Select value={assignmentId} onValueChange={setAssignmentId}>
            <SelectTrigger className="h-10 rounded-xl sm:min-w-[14rem]">
        ...
        <Button type="button" variant="outline" size="sm" className="rounded-xl shrink-0" onClick={() => setShowNames((v) => !v)}>
          {showNames ? "Hide student names" : "Reveal student names"}
        </Button>
      </div>
          ...
          <TableBody>
            {subsLoading ? (
              ...
              filtered.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className="tabular-nums">{r.roll}</TableCell>
                  <TableCell>{showNames ? r.studentName : `Student #${r.roll}`}</TableCell>
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={cn(
                        ...
                        (r.timing === "on_time" || r.timing === "early") && "border-success/30 text-success",
                      )}
                    >
                  ...
                  <TableCell className="max-w-[14rem] truncate text-sm">{r.note || "—"}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
...
function NewAssignmentDialog({
  ...
  const createFn = useCallback(async (data: NewAssignmentForm) => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
            ...
            <DialogFooter className="gap-2">
              ...
              <Button type="submit" disabled={creating}>{creating ? "Saving…" : "Save draft"}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
