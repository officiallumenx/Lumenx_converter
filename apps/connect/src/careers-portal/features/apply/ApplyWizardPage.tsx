...
import { Badge, Button, Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Textarea } from "@lumenx/ui";
...
export function ApplyWizardPage({ jobId }: { jobId?: string }) {
  ...
  const selectedJob = useMemo(() => (activeJobId ? getJobById(activeJobId) : undefined), [activeJobId]);
  ...
  useEffect(() => {
    ...
    const built = buildApplicationDraftFromProfile(jobId, profile, user.name, user.email, user.phone);
  ...
  const missing = profile && user && draft ? getMissingQuickApplyFields(profile, user.email, user.phone, draft) : [];

  const needsExtras = selectedJob ? jobHasApplicationExtras(selectedJob) : false;



  const update = (patch: Partial<ApplicationDraft>) => setDraft((d) => (d ? { ...d, ...patch } : d));
  ...
  const handleSubmit = () => {
    ...
    const docTypes: CareerDocumentType[] = ["resume", "certificates", "experience_letters", "identity_proof", "profile_photo", "demo_teaching_video", "additional"];
    ...
    return (
      ...
      <div className="mx-auto max-w-lg text-center animate-in fade-in py-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          ...
          Your application for {selectedJob?.title} at {selectedJob?.instituteName} has been received.

        </p>

        <p className="mt-4 rounded-xl bg-muted/60 px-4 py-3 font-mono text-sm">Application ID: {submittedId}</p>

        <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-center">

          <Button asChild><Link to="/careers/applications/$applicationId" params={{ applicationId: submittedId }}>Track application</Link></Button>

          <Button variant="outline" asChild><Link to="/careers/jobs">Browse more jobs</Link></Button>
    ...
    <div className="animate-in fade-in duration-300 max-w-2xl">
          ...
          >
            ...
            <SelectContent>
              ...
              {jobs.map((j) => (
                ...
                <SelectItem key={j.id} value={j.id}>{j.title} — {j.instituteName}</SelectItem>
        ...
        <div className="mb-6 rounded-2xl border border-border p-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-muted-foreground">{selectedJob.instituteName} · {selectedJob.department}</p>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <Badge variant="outline" className="capitalize">{selectedJob.employmentType.replace(/_/g, " ")}</Badge>
      ...
      {selectedJob && missing.length === 0 && (
        ...
        <div className="mb-6 rounded-2xl border border-border bg-muted/30 p-4 space-y-3">
          ...
          <p className="text-sm font-medium flex items-center gap-2"><User className="size-4 text-primary" /> Applying as</p>
          ...
          <div className="grid gap-2 text-sm sm:grid-cols-2">
            ...
            <p><span className="text-muted-foreground">Mobile:</span> {user.phone}</p>

            <p><span className="text-muted-foreground">Location:</span> {profile.city}, {profile.state}</p>

            <p className="sm:col-span-2 flex items-center gap-1">

              <FileText className="size-3.5 text-muted-foreground" />

              <span className="text-muted-foreground">Resume:</span> {profile.resumeFileName ?? "On profile"}

            </p>

            {profile.headline && (

              <p className="sm:col-span-2"><span className="text-muted-foreground">Headline:</span> {profile.headline}</p>
        ...
        <div className="mb-6 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 space-y-4">
          ...
          {missing.includes("email") && (
            ...
            <div className="space-y-2">
              ...
              <Input type="email" value={draft.personal?.email ?? ""} onChange={(e) => update({ personal: { ...draft.personal, email: e.target.value } })} />
          ...
          {missing.includes("mobile") && (
            ...
            <div className="space-y-2">
              ...
              <Input value={draft.personal?.mobile ?? ""} onChange={(e) => update({ personal: { ...draft.personal, mobile: e.target.value } })} />
          ...
          {missing.includes("city") && (
            ...
            <div className="space-y-2">
              ...
              <Input value={draft.address?.city ?? ""} onChange={(e) => update({ address: { ...draft.address, city: e.target.value } })} />
          ...
          {missing.includes("state") && (
            ...
            <div className="space-y-2">
              ...
              <Input value={draft.address?.state ?? ""} onChange={(e) => update({ address: { ...draft.address, state: e.target.value } })} />
          ...
          {missing.includes("resume") && (
            ...
            <DocumentUploadCard
              ...
              onUpload={(f) => update({ documents: { ...draft.documents, resume: { fileName: f.name } } })}
        ...
        <div className="mb-6 rounded-2xl border border-border p-4 space-y-4">
          ...
          {selectedJob.applicationExtras?.demoVideo && (
            ...
            <DocumentUploadCard
              ...
              onUpload={(f) => update({ documents: { ...draft.documents, demo_teaching_video: { fileName: f.name } } })}
      ...
      {selectedJob && (
        ...
        <Button size="lg" className="w-full sm:w-auto" onClick={handleSubmit} disabled={missing.length > 0}>
