  ...
  return (
    <div className="animate-in fade-in duration-300 space-y-6">
      ...
      <SectionCard title={`Candidates (${talent.length})`}>
        {talent.length === 0 ? (
          <p className="text-sm text-muted-foreground py-8 text-center">No talent matches your search.</p>
        ) : (
            ...
            {talent.map((c) => (
              <div key={c.candidateId} className="rounded-xl border border-border p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <ProfileStrengthBadge strength={c.profileComplete >= 80 ? "Strong" : "Developing"} percent={c.profileComplete} />
                </div>
                <p className="text-xs text-muted-foreground">{c.city}, {c.state} · {c.experienceYears} yrs exp</p>
