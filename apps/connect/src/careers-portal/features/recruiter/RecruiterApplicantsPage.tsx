...
import { Badge, Button, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@lumenx/ui";
...
export function RecruiterApplicantsPage() {
  ...
  const apps = useMemo(
    () => getApplicationsForOrganization(user.organizationId!),
    ...
    <div className="animate-in fade-in duration-300 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <SectionCard title={`Applicants (${filtered.length})`} className="lg:col-span-3">
          {filtered.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">No applications yet for your open roles.</p>
          ) : (
          ...
          {selected ? (
            <SectionCard title="Application detail">
              <div className="space-y-3 text-sm">
                <p><span className="text-muted-foreground">Candidate:</span> {selected.personal.name}</p>
                <p><span className="text-muted-foreground">Role:</span> {selected.jobTitle}</p>
                <p><span className="text-muted-foreground">Email:</span> {selected.personal.email}</p>
                <p><span className="text-muted-foreground">Experience:</span> {selected.professional.experienceYears} yrs</p>
                <p><span className="text-muted-foreground">Qualification:</span> {selected.professional.highestQualification}</p>
                <Badge variant={statusTone(selected.status)}>{statusLabel(selected.status)}</Badge>
                <ApplicationTimelineV2 events={selected.timeline.slice(-5).map((e) => ({
                  id: e.id,
                  status: e.status,
                  label: e.label,
                  at: e.at,
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground truncate">{app.jobTitle} · {app.id}</p>
      </div>
