...
export function RecruiterWorkspacePage() {
  ...
  const inInterview = apps.filter((a) => a.status === "interview_scheduled" || a.status === "interview_completed").length;
  const offers = apps.filter((a) => a.status === "offer_sent" || a.status === "offer_accepted").length;
  ...
  return (
    <div className="animate-in fade-in duration-300 space-y-8">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="In interview" value={String(inInterview)} icon={FolderOpen} tone={inInterview > 0 ? "warning" : "default"} />
        <StatCard label="Offers" value={String(offers)} icon={Sparkles} tone={offers > 0 ? "success" : "default"} />
      </div>

      <SectionCard title="Quick actions">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <Button className="h-auto py-3 justify-start" asChild>
            <Link to="/careers/recruiter/jobs/new"><Plus className="size-4 mr-2 shrink-0" />Post a job</Link>
          </Button>
          <Button variant="outline" className="h-auto py-3 justify-start" asChild>
            <Link to="/careers/recruiter/jobs"><Briefcase className="size-4 mr-2 shrink-0" />Manage listings</Link>
          </Button>
          <Button variant="outline" className="h-auto py-3 justify-start" asChild>
            <Link to="/careers/jobs"><Briefcase className="size-4 mr-2 shrink-0" />Browse market</Link>
          </Button>
          <Button variant="outline" className="h-auto py-3 justify-start" asChild>
            <Link to="/careers/recruiter/applicants"><FolderOpen className="size-4 mr-2 shrink-0" />Applicant pipeline</Link>
          </Button>
          <Button variant="outline" className="h-auto py-3 justify-start" asChild>
            <Link to="/careers/recruiter/talent"><Users className="size-4 mr-2 shrink-0" />Discover talent ({talent.length})</Link>
          </Button>
        </div>
      </SectionCard>

      {apps.length > 0 && (
        <SectionCard title="Recent applicants" link="/careers/recruiter/applicants" linkLabel="View pipeline">
          <div className="space-y-2">
            {apps.slice(0, 5).map((app) => (
              <div key={app.id} className="flex items-center justify-between rounded-xl border border-border p-3 text-sm">
