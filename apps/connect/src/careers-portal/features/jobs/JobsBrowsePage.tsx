...
import { getFeaturedJobs, getTrendingJobs, getRecentJobs, getRecommendedJobs } from "@/lib/careers/recommendations";
...
export function JobsBrowsePage() {
  ...
  const recommended = useMemo(
    () => (profile ? getRecommendedJobs(profile, 3) : []),
    [profile],
  );
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] lg:h-[calc(100vh-4rem)] min-h-0">
      <CareersPageHeader
        ...
        backTo={recruiter ? "/careers/recruiter" : undefined}
        action={
          recruiter ? (
            <Button size="sm" asChild>
              <Link to="/careers/recruiter/jobs/new"><Plus className="size-4 mr-1.5" /> Post a job</Link>
            </Button>
          ) : undefined
        }
      />
      ...
      <div className="sticky top-0 z-10 -mx-4 px-4 py-3 bg-background/95 backdrop-blur border-b border-border space-y-3 shrink-0">
        <div className="relative">
          ...
          <Input
            className="pl-9"
            placeholder={recruiter ? "Search roles, companies, skills…" : "Search job title, company, or skills…"}
            value={q}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          <Select value={sort} onValueChange={(v) => setSort(v as typeof sort)}>
            <SelectTrigger className="w-[110px] shrink-0"><SelectValue placeholder="Sort" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={experience} onValueChange={(v) => setExperience(v as ExperienceBand)}>
            <SelectTrigger className="w-[120px] shrink-0"><SelectValue placeholder="Experience" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All experience</SelectItem>
              {(Object.entries(EXPERIENCE_BAND_LABEL) as [Exclude<ExperienceBand, "all">, string][]).map(([k, label]) => (
                <SelectItem key={k} value={k}>{label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Select value={workMode} onValueChange={setWorkMode}>
            <SelectTrigger className="w-[120px] shrink-0"><SelectValue placeholder="Work mode" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={category} onValueChange={(v) => setCategory(v as JobCategory | "all")}>
            <SelectTrigger className="w-[140px] shrink-0"><SelectValue placeholder="Role type" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={state} onValueChange={setState}>
            <SelectTrigger className="w-[120px] shrink-0"><SelectValue placeholder="State" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={city} onValueChange={setCity}>
            <SelectTrigger className="w-[120px] shrink-0"><SelectValue placeholder="City" /></SelectTrigger>
            <SelectContent>
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto overscroll-contain mt-4 space-y-8 min-h-0 pb-4">
        {showSections && (
          <>
            {!recruiter && recommended.length > 0 && (
              <section>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {recommended.map(({ job }) => <JobCard key={job.id} job={job} compact {...cardProps} />)}
                </div>
              </section>
            )}
            <section>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {featured.slice(0, 4).map((j) => <JobCard key={j.id} job={j} compact {...cardProps} />)}
              </div>
            </section>
            <section>
              <h2 className="font-display text-sm font-bold flex items-center gap-2 mb-3">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {trending.slice(0, 4).map((j) => <JobCard key={j.id} job={j} compact {...cardProps} />)}
              </div>
            </section>
            <section>
              <h2 className="font-display text-sm font-bold flex items-center gap-2 mb-3">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {recent.map((j) => <JobCard key={j.id} job={j} compact {...cardProps} />)}
              </div>
            </section>
          </>
        )}
        ...
        <section>
          <h2 className="font-display text-sm font-bold mb-3">
            {hasFilters ? `Results (${jobs.length})` : recruiter ? "All market listings" : "All jobs"}
          </h2>
          {jobs.length === 0 ? (
            <p className="text-center text-sm text-muted-foreground py-12">No jobs match your filters.</p>
          ) : (
            <div className="space-y-4">{jobs.map((j) => <JobCard key={j.id} job={j} {...cardProps} />)}</div>
          )}
        </section>
      </div>
    </div>
