...
import { ApplicationTimelineV2, ApplicationProgressTracker, DocumentVerificationCard, DemoClassCard } from "@/careers-portal/shared/ui/v2/CareersV2Widgets";
...
import { getStatusProgress, interviewModeLabel, statusLabel, statusTone } from "@/lib/careers/status-utils";
...
export function ApplicationsListPage() {
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <CareersPageHeader title="My applications" subtitle={`${apps.length} application${apps.length === 1 ? "" : "s"}`} />
          ...
          {apps.map((a) => (
            ...
            >
              ...
              <div className="mt-3 h-1.5 rounded-full bg-muted overflow-hidden">
                <div className="h-full bg-primary rounded-full" style={{ width: `${getStatusProgress(a.status)}%` }} />
              </div>
              <p className="mt-2 text-xs text-muted-foreground">Updated {new Date(a.updatedAt).toLocaleDateString("en-IN")}</p>
            </Link>
          ))}
        </div>
      )}
...
export function ApplicationDetailPage({ applicationId }: { applicationId: string }) {
  ...
  if (!app) {
    return (
      <div className="text-center py-12">
        ...
        <Button className="mt-4" asChild><Link to="/careers/applications">Back to list</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <CareersPageHeader title={app.jobTitle} subtitle={`${app.instituteName} · ${app.id}`} backTo="/careers/applications" />

      <Badge variant={statusTone(app.status)} className="mb-4">{statusLabel(app.status)}</Badge>
      ...
      {app.interview && (
        <section className="mb-8 rounded-2xl border border-border p-4">
          ...
          <div className="space-y-2 text-sm">
            <p className="flex items-center gap-2"><Calendar className="size-4" /> {app.interview.date} at {app.interview.time}</p>
            <p className="flex items-center gap-2">
              {app.interview.mode === "video" ? <Video className="size-4" /> : <MapPin className="size-4" />}
            ...
            <Badge variant="outline" className="capitalize">{app.interview.status}</Badge>
          </div>
        </section>
      )}
      ...
      {app.hrNotes && app.hrNotes.length > 0 && (
        <section className="mb-8">
          ...
          <ul className="space-y-2">
            {app.hrNotes.map((n, i) => (
              <li key={i} className="rounded-xl bg-muted/50 p-3 text-sm text-muted-foreground">{n}</li>
            ))}
          </ul>
        </section>
      )}
